...
//		testDeleteCountry();
		testSearchCountry();
		testSearchCountryWithPrefix();
	}

	private static void testSearchCountry(){
		LOGGER.info("Start");
		List<Country> list = countryService.searchCountryByKeyword("ou");
		for(Country c: list){
	...
	private static void testSearchCountryWithPrefix(){
		LOGGER.info("Start");
		List<Country> list = countryService.searchCountryWithPrefix("Z");
		for(Country c: list){
			LOGGER.debug("Country:{}", c);
		}
		LOGGER.info("End");
